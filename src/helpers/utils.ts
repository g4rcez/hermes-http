export type QueryString<T> = { [key in keyof T]: T[key] };
...
export const concatUrl = (url: string, uri?: string) =>
	uri ? `${url.replace(/\/+$/, "").trim()}/${uri.replace(/^\/+/, "").trim()}` : url;
...
export const qs = <T>(args: QueryString<T>, opt: Options = { encode: true, strict: true }) => {
	const nonNull = Object.entries(args).reduce(
		(acc, [key, value]: [any, any]) => ((args as any)[key] ? { ...acc, [key]: value } : acc) as string,
		{}
	);
